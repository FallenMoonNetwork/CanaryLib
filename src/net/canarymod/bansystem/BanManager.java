...
import net.canarymod.backbone.IBackboneBans;
...
public class BanManager {
    private IBackboneBans backbone;
    private ArrayList<Ban> bans = new ArrayList<Ban>();
    
    public BanManager(IBackbone bone, IBackbone.Type type) {
        backbone = (IBackboneBans) bone.getBackbone(IBackbone.System.BANS, type);
     ...
     * @param reason
     */
    public void issueBan(IPlayer player, String reason) {
     ...
     * @param player
     * @param reason
     * @param time The time component must be NUMBER HOUR/DAY/WEEK/MONTH. <br>
     * Example:
     * /ban player Being incredibly stupid 5 HOURS
     * If you put nothing as time unit, it will evaluate as HOURS!
     */
    public void issueBan(IPlayer player, String reason, String time) {
        long timeToAdd = 0L;
        try {
        ...
        Ban ban = new Ban(player, reason, ICanary.getUnixTimestamp()+timeToAdd, false);
        bans.add(ban);
        backbone.addBan(ban);
    }
     ...
     * @param reason
     */
    public void issueIpBan(IPlayer player, String reason) {
     ...
     * @param time
     */
    public void issueIpBan(IPlayer player, String reason, String time) {
        long timeToAdd = 0L;
        try {
            timeToAdd = parseTimeSpec(time);
        }
        ...
        Ban ban = new Ban(player, reason, ICanary.getUnixTimestamp()+timeToAdd, true);
        bans.add(ban);
        backbone.addBan(ban);
    }
    
    /**
     * Check if banned and unban if ban has expired.
     * Returns true if still banned, false otherwise.
     * THIS WILL ALSO WORK FOR IP!
     * @param subject
     * @return
     */
     ...
     * @param player
     */
    public void unban(IPlayer player) {
        Ban test = null;
        for(Ban b : bans) {
            if(!b.isIpBan()) {
     ...
     * formatted like this: number hours|days|months Ex: 1 month and it
     * will return the amount of seconds that contain one month
     * 
     * @param ts
